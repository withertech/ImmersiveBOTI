...
public class ClippedBlockView implements IBlockReader {
    ...
    public boolean isClipped(BlockPos pos) {
        return Vector3d.copyCentered(pos).subtract(clipPos).dotProduct(contentDirection) < 0;
    }
    ...
    public BlockRayTraceResult rayTraceBlocks(RayTraceContext context) {
        Vector3d delta = context.getEndVec().subtract(context.getStartVec());
        ...
        Vector3d startPos = context.getStartVec().add(delta.scale(t));
