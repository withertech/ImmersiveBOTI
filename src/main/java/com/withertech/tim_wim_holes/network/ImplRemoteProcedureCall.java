...
public class ImplRemoteProcedureCall
{
	...
	static
	{
		...
		serializerMap = ImmutableMap.<Class, BiConsumer<PacketBuffer, Object>>builder()
				...
				.put(RegistryKey.class, (buf, o) -> buf.writeResourceLocation(((RegistryKey) o).getLocation()))
		...
		deserializerMap = ImmutableMap.<Type, Function<PacketBuffer, Object>>builder()
				.put(ResourceLocation.class, PacketBuffer::readResourceLocation)
				.put(
						new TypeToken<RegistryKey<World>>()
						{
						}.getType(),
						buf -> RegistryKey.getOrCreateKey(
								Registry.WORLD_KEY, buf.readResourceLocation()
						)
				)
				.put(
						new TypeToken<RegistryKey<Biome>>()
						{
						}.getType(),
						buf -> RegistryKey.getOrCreateKey(
								Registry.BIOME_KEY, buf.readResourceLocation()
						)
				)
				.put(BlockPos.class, PacketBuffer::readBlockPos)
				.put(Vector3d.class, buf ->
						new Vector3d(buf.readDouble(), buf.readDouble(), buf.readDouble())
				)
				.put(UUID.class, PacketBuffer::readUniqueId)
				...
				.put(CompoundNBT.class, PacketBuffer::readCompoundTag)
				.put(ITextComponent.class, PacketBuffer::readTextComponent)
				.build();
	}
	...
	private static String readStringNonClientOnly(PacketBuffer buf)
	{
	...
	private static Method findMethodByPath(String methodPath)
	{
		...
		return Arrays.stream(aClass.getMethods()).filter(
