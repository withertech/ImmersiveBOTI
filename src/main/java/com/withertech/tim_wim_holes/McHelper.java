...
public class McHelper
{
	...
	{
		return (IEThreadedAnvilChunkStorage) getServer().getWorld(dimension).getChunkProvider().chunkManager;
	}
	...
	)
	{
		...
		return world.getEntitiesWithinAABB(entityClass, box, e -> true).stream();
	}
	...
	{
		ChunkHolder chunkHolder_ = ((IEThreadedAnvilChunkStorage) world.getChunkProvider().chunkManager).getChunkHolder_(ChunkPos.asLong(x, z));
		if (chunkHolder_ == null)
		{
			return null;
		}
	...
	{
		return new StringTextComponent(link).modifyStyle(
				style -> style.setClickEvent(new ClickEvent(
	...
	public static Iterable<Entity> getWorldEntityList(World world)
	{
		...
		} else
		{
			...
			{
				return ((ServerWorld) world).getEntitiesIteratable();
			} else
			{
		...
		for (Map.Entry<RegistryKey<T>, T> entry : registry.getEntries())
		{
			...
			if (predicate.test(key, object))
			{
				newRegistry.register(
						key, object, registry.getLifecycleByRegistry(object)
				);
			}
		}
	...
	public static ServerWorld getServerWorld(RegistryKey<World> dim)
	{
		...
		if (world == null)
		{
			throw new RuntimeException("Missing dimension " + dim.getLocation());
		}
		return world;
	}
	...
	public static ITextComponent tagToTextSorted(INBT tag, String indent, int depth)
	{
		...
		if (tag instanceof ByteNBT)
		{
			...
			if (value == 1)
			{
				return new StringTextComponent("true").mergeStyle(TextFormatting.GOLD);
			} else if (value == 0)
			{
				return new StringTextComponent("false").mergeStyle(TextFormatting.GOLD);
			}
		}
	...
	public static ITextComponent compoundTagToTextSorted(CompoundNBT tag, String indent, int depth)
	{
		...
		} else
		{
			...
			for (Iterator iterator = ((Collection) collection).iterator(); iterator.hasNext(); mutableText.appendSibling(mutableText2))
			{
				String keyName = (String) iterator.next();
				mutableText2 = (new StringTextComponent(Strings.repeat(indent, depth + 1)))
						.appendSibling(prettyPrintTagKey(keyName))
				...
				if (iterator.hasNext())
				{
					mutableText2.appendString(String.valueOf(',')).appendString(indent.isEmpty() ? " " : "\n");
				}
			}

			if (!indent.isEmpty())
			{
				mutableText.appendString("\n").appendString(Strings.repeat(indent, depth));
			}
