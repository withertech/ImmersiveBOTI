package com.withertech.imm_boti.dimension_sync;
...
public class DimensionIdManagement {
    ...
    private static File getIPDimIdFile() {
        ...
        Path saveDir = server.anvilConverterForAnvilFile.saveDir;
    ...
    private static void completeServerIdRecord() {
        ...
        if (!bimap.containsKey(World.THE_NETHER)) {
            bimap.put(World.THE_NETHER, -1);
        }
        if (!bimap.containsKey(World.THE_END)) {
            bimap.put(World.THE_END, 1);
        }
