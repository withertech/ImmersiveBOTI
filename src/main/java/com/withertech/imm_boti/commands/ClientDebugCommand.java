package com.withertech.imm_boti.commands;
...
import com.withertech.imm_boti.chunk_loading.ChunkVisibility;
...
import com.withertech.imm_boti.network.McRemoteProcedureCall;
import com.withertech.imm_boti.optifine_compatibility.UniformReport;
            ...
            .executes(context -> {
                ...
                Minecraft.getInstance().execute(() -> {
                    ClientWorldLoader.getClientWorlds().forEach((world) -> {
                        ...
                        McHelper.serverLog(
                            player,
                            world.getDimensionKey().getLocation().toString() + builtChunkStorage.getDebugString()
                        );
                    });
                });
...
//                );
        ...
        builder.then(Commands
            ...
            .executes(context -> {
                for (ClientWorld clientWorld : ClientWorldLoader.getClientWorlds()) {
                    ...
                    McHelper.serverLog(player, "\n" + clientWorld.getDimensionKey().getLocation().toString());
                    result.forEach((g, l) -> {
                        McHelper.serverLog(player, "\n" + g.toString());
    ...
    public static void invokeEraseChunk(World world, ChunkPos center, int r, int downY, int upY) {
        ...
        poses.sort(Comparator.comparingDouble(c ->
            Vector3d.copy(center.asBlockPos()).distanceTo(Vector3d.copy(c.asBlockPos()))
        ));
        ...
        McHelper.getServer().getWorlds().forEach(
            world -> {
                str.append(String.format(
                    "%s %s\n",
                    world.getDimensionKey().getLocation(),
                    NewChunkTrackingGraph.getLoadedChunkNum(world.getDimensionKey())
                ));
            }
        );
    ...
    private static int listPortals(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        playerServer.getServer().getWorlds().forEach(world -> {
            result.append(world.getDimensionKey().getLocation() + "\n");
            for (Entity e : world.getEntitiesIteratable()) {
    ...
    private static int reportPlayerStatus(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        McHelper.serverLog(
            ...
            String.format(
                ...
                playerMP.world.getDimensionKey().getLocation(),
        ...
        McHelper.serverLog(
            ...
            String.format(
                ...
                playerSP.world.getDimensionKey().getLocation(),
    ...
    private static void testRemoteProcedureCall(ServerPlayerEntity player) {
        ...
        McHelper.getServer().execute(() -> {
            McRemoteProcedureCall.tellClientToInvoke(
                ...
                World.THE_NETHER, Biomes.JUNGLE,
                new BlockPos(3, 5, 4),
                new Vector3d(7, 4, 1)
            );
        });
